...
def test_price_shows_a_red_dot_and_no_duplicate_warning_for_confirmed_stale_data(tmp_path):
    """The freshness dot already carries the age signal for a displayed SCU figure, so the
    separate ⚠️ health-warning note must not also be appended to the same line - that
    would repeat the same information twice in one line."""
    async def run():
...
def test_price_hides_buying_capacity_when_status_confirms_no_demand(tmp_path):
    ...
    same buy/sell status inversion this codebase has hit before."""
    async def run():
