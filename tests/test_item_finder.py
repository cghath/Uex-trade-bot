...
from bot.uex.item_finder import format_item_listing_line, location_breadcrumb, rank_item_listings
...
def test_location_breadcrumb_full_chain():
    row = _row(
        10, "Dumper's Depot", 100,
        star_system_name="Stanton", planet_name="ArcCorp", city_name="Area18",
    )
    assert location_breadcrumb(row) == "Stanton → ArcCorp → Area18 → Dumper's Depot"


def test_location_breadcrumb_skips_missing_levels():
    row = _row(10, "Some Outpost", 100, star_system_name="Pyro", outpost_name=None, city_name=None)
    assert location_breadcrumb(row) == "Pyro → Some Outpost"
...
def test_location_breadcrumb_prefers_moon_over_orbit_and_outpost_over_space_station():
    row = _row(
        ...
        star_system_name="Stanton", orbit_name="Crusader", moon_name="Aberdeen",
        space_station_name="Some Station", outpost_name="Klescher",
    )
    assert location_breadcrumb(row) == "Stanton → Aberdeen → Klescher → Klescher Trading Post"
...
def test_rank_item_listings_unknown_distance_sorts_last_not_first():
    """A terminal /terminals_distances couldn't price (e.g. cross-system, or a lookup
    failure) must never be treated as distance 0 - that would wrongly bury a genuinely
    close option behind it."""
...
def test_rank_item_listings_carries_the_location_label_and_price_through():
    ...
    assert ranked[0].location_label == "Stanton → Dumper's Depot"
...
def test_format_item_listing_line_shows_distance_and_price():
    from bot.uex.item_finder import ItemListing
    listing = ItemListing(id_terminal=1, terminal_name="X", location_label="Stanton → X", price_buy=1500, distance_gm=3.25)
...
def test_format_item_listing_line_zero_distance_says_already_here():
    from bot.uex.item_finder import ItemListing
    listing = ItemListing(id_terminal=1, terminal_name="X", location_label="X", price_buy=100, distance_gm=0.0)
    assert "you're already here" in format_item_listing_line(listing)


def test_format_item_listing_line_unknown_distance_says_so():
    from bot.uex.item_finder import ItemListing
    listing = ItemListing(id_terminal=1, terminal_name="X", location_label="X", price_buy=100, distance_gm=None)
    assert "distance unknown" in format_item_listing_line(listing)
...
def _cog(*, resolved_terminal, catalog, items_prices, distance_by_pair=None):
    ...
    cog.bot = NS(
        db=NS(resolve_terminal_id_by_name=AsyncMock(return_value=resolved_terminal)),
        uex=NS(
...
def test_ingame_item_finder_happy_path_sorts_closest_first():
    async def run():
        cog = _cog(
    ...
    shops_field = next(f for f in embed.fields if f.name == "Shops")
    lines = shops_field.value.splitlines()
    assert lines[0].startswith("**Stanton → Near Armory**"), "closest terminal must be listed first"
...
def test_ingame_item_finder_origin_terminal_itself_never_gets_a_live_distance_call():
    ...
    embed = interaction.followup.send.call_args.kwargs["embed"]
    shops_field = next(f for f in embed.fields if f.name == "Shops")
    assert "you're already here" in shops_field.value


def test_ingame_item_finder_a_distance_lookup_failure_still_shows_the_listing():
    """A single failed /terminals_distances call (network hiccup, an unpriceable pair)
    must not crash the whole command or silently drop that shop - it shows with 'distance
    unknown' and sorts after every successfully-measured option."""
    async def run():
        cog = _cog(
            resolved_terminal=(1, "Area18 TDD"),
            catalog=[{"id": 5, "name": "P4-AR"}],
            items_prices=[
                ...
                _row(20, "Normal Shop", 1300, item_name="P4-AR", star_system_name="Stanton"),
            ],
            distance_by_pair={(1, 20): {"distance": 5.0}},  # (1, 10) deliberately absent -> None
        )
        interaction = _FakeInteraction()
...
def test_ingame_item_finder_truncates_and_discloses_omitted_count():
    async def run():
        ...
        cog = _cog(
            ...
            items_prices=items_prices, distance_by_pair=distance_by_pair,
        )
        interaction = _FakeInteraction()
    ...
    embed = interaction.followup.send.call_args.kwargs["embed"]
    shops_field = next(f for f in embed.fields if f.name == "Shops")
    assert len(shops_field.value.splitlines()) == MAX_RESULTS_SHOWN
